...
<div class="page-header">
    <h1><span class="text-light-gray">教师与机构 / </span>修改教师</h1>
</div> <!-- / .page-header -->
<div class="panel">
    @if (\Session::get('tips'))
...
<div class="row">
    <div class="col-sm-12">
        <form class="panel form-horizontal" method="POST" enctype="multipart/form-data" action="{{route('adminTeacherModifyAction')}}">
            <div class="panel-heading">
                <span class="panel-title">教师资料</span>
            </div>
                    ...
                    <div class="col-sm-4">
                        <select class="form-control" name="department">
                            @foreach($departments as $parent)
                                <option value="{{$parent->id}}" style="color: #000" {{$objDepartment->id == $parent->id ? 'selected' :''}}>{{$parent->name}}</option>
                                    @foreach($parent->child as $ch)
                                        <option value="{{$ch->id}}" {{$objDepartment->id == $ch->id ? 'select' :''}}> |--{{$ch->name}}</option>
                                    @endforeach
                            @endforeach
                        </select>
                    </div>
